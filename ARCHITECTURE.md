...
`data/knowledge/source-records.json` is only the fingerprint-locked index of 688 approved rows and their 717 required components. `data/books/manifest.json` is a generated, queue-only public aggregate whose ordered entries preserve every component identity and whose 717 `artifactPath` values remain null. Locally populated components use the canonical ignored path `data/books/S####/<component>.json`; their existence never changes the tracked manifest. Five local candidate graphs use the v1.1 sharded representation described below, while Git stores only their source-resolution metadata and hashes.
...
Every shard index entry records its relative path, schema version, record count, byte count, and SHA-256 digest. A component digest covers the logical digest, distribution policy, metadata, and ordered shard descriptors. The first migration preserves every existing per-entity evidence object and logical record exactly rather than deduplicating reviews or rewriting their subjects. The tracked root manifest contains exactly 717 registry-derived component rows and zero payload paths; ignored local indexes retain the five candidate component digests without losing queue identity.

Git currently stores schemas, queue manifests, source pins, candidate extraction/graph hashes, review decisions, and small authored metadata; it stores no raw textbook graph index or shard. The five current graphs remain at ignored local paths, where checks reconstruct and cross-check them when present. Each component declares whether its derived records may be distributed. Before multi-node extraction, a v1.2 placement layer must use stable source-unit or stable-ID/hash-prefix partitions and keep immutable content-addressed shards in license-appropriate private object storage or local caches while Git retains digests, review records, and small mergeable metadata. SQLite, DuckDB, and search indexes may be generated for workers or analysis, but they are disposable derivatives and never the review authority.
...
Raw graph shards are ignored by Git, not copied into GitHub Pages, and refused by the development server. A Git-index boundary check fails if any component index or shard is force-added. If public graph dumps are later useful, a release or object store will publish versioned compressed snapshots with the same component and shard digests, and only after the relevant distribution decision is approved. The local S0060, S0091, S0164, S0262, and S0321 candidates have exact logical round-trip parity under the codec: 6, 3, 6, 17, and 6 shards respectively, with 38 total shards and a largest shard of 5,242,802 bytes. Importers read either legacy v1 or v1.1, refuse tracked or non-ignored write destinations, and perform single-writer local updates that are rollback-safe for ordinary errors; checks reject altered or orphan local shards whenever the cache is present.
...
SourceRegistry ── indexes ──> BookGraphManifest
  └─ SourceRecord               └─ one BookGraph entry per required component
       └─ required components        └─ public artifactPath -> null
                                      local candidate -> ignored data/books/S####/<component>.json
...
BookGraph
  ...
  ├─ SourceArtifactNode
  │    ├─ exact proof-referenced section / equation / item / remark / example / exercise / prose span
  │    ├─ source hash and locator, but no theorem/support classification
  │    └─ original-route placeholder pending promotion, decomposition, or nondependency review
  ├─ SourceDependencyEdge
  │    ├─ mathematical node, raw source artifact, or external input -> dependent theorem
...
- the generated manifest to index all 717 required components exactly once, with a safe unique path and identity matching its immutable source row/component; v1.2 reports source-route gaps separately from mathematical-dependency-pending theorems and raw source-artifact counts;
...
- every extracted source unit to have an evidence-bound inventory listing its theorem, mathematical-support, and raw source-artifact nodes or explicitly attesting that the unit is theorem-free; reviewed extraction requires independent review of every unit decision;
- every theorem-like source occurrence, supporting mathematical node, and raw source artifact to retain a permanent local ID, kind, exact label, source unit, and locator;
...
- a graph-complete theorem-like node to have a reviewed mathematical route with reviewed dependencies, or an explicit reviewed root/external-input attestation; an empty captured source route or a route to a raw source artifact cannot satisfy this gate;
- a `reviewed-complete` graph to contain no unresolved raw source artifacts; each must first be promoted, decomposed, or closed as an occurrence-specific nondependency;
...
The publication build recomputes the ordered source registry, the 717-entry exact-source queue, the queue-only 717-component book manifest, every current Knowledge-node content fingerprint, every administrative review's subject fingerprint, and the rule that Git tracks no raw book graph. Local checks additionally reconstruct every available ignored candidate and compare its edition and importer hashes with the source ledger. A stale source, graph, target, approval, or publication boundary therefore fails instead of inheriting an old reviewed state.

The current 688-row / 717-component registry is intentionally incomplete. S0060, S0091, S0164, S0262, and S0321 contain five pinned candidate extractions in ignored local sharded v1.1 storage; all 717 tracked manifest entries have null artifact paths. All five have tracked candidate exact-edition and importer-hash records, while zero editions, distributions, extraction decisions, dependencies, or complete graphs are independently reviewed. S0060 exercises the original PreTeXt `.ptx` boundary; S0091 and S0164 extend it to `.xml` content roots while excluding embedded PreFigure asset XML and graph-like markup nested in examples, exercises, activities, remarks, and other pedagogical containers. S0091 captures 103 nodes but no explicit proof-xref edge, so its 65 theorem-like nodes remain dependency-pending. S0164 captures 348 nodes and 70 explicit proof-xref candidates, with 21 unresolved xrefs and an open ambiguity between GFDL-1.2-or-later and GFDL-1.3-or-later markers in the active source closure.

S0262 is the first and highest-priority book under the descending-complexity work order. Its raw source-proof/xref pass is materialized. Against the permanent Stacks tag system it contains 116 source units; 13,157 theorem-like nodes; 1,894 mathematical support nodes; 1,237 separately classified raw source artifacts; 11 typed external theorems; 38,904 raw candidate edges; and 13,201 captured source or alternate proof routes. The explicit 38,644 proof-xref edges include 2,435 `source-reference` provenance edges to raw artifacts. Another 260 owner-specific audited semantic edges comprise 71 named-result, 6 curated-claim, 12 external-citation, 109 deictic-proof, 25 recall-bundle, and 37 section-delegation dependencies. All tagged proof xrefs are structurally accounted for and all theorem-like nodes have a captured source route; 1,737 empty source routes record proof coverage without declaring roots. Six bibliographic proof citations remain unresolved, 1,883 theorem-like nodes still lack a route with a mathematical prerequisite, and every raw artifact still needs promotion, decomposition, or occurrence-specific suppression. The raw pass is reproducible, but no extraction, mathematical dependency, or complete-graph claim is independently reviewed.

S0321 is the second operational target. Its complete-build importer evaluates the official Open Logic tag configuration and ordered `\olimport` expansion rather than globbing physical files. The resulting 694 import instances cover 642 unique paths and retain 52 intentional duplicate-context instances so PL and FOL namespaces and active text remain distinct. The raw graph contains 786 theorem-like nodes, 522 support nodes, 155 exact proof-reference artifacts, 700 candidate dependencies, 693 source-proof routes, and 892 references. Eighteen references remain unresolved and 539 theorem-like nodes remain dependency-pending. The exact formal-environment baseline was cross-checked against TeX execution; the parser handles an unbraced conditional control-sequence argument that otherwise creates a false PL duplicate. Missing imports, cycles, and unevaluated conditionals are all zero. These are reproducible extraction facts, not independent mathematical or administrative review.

The preceding exact section-delegation batch maps owners `0BB4`, `06QZ`, `07SK`, and `0DUJ` from Section Tag `0261` to the particular quotient/coequalizer result in Tag `0262`, respectively for the displayed étale-groupoid quotient, $X=U/R$, $Y=W/R'$, and gluing the compatible $h_i$ into a unique $Y\to W$. It also maps `09CN`'s Section Tag `07JV` invocation to the snake lemma in Tag `07JW`. Every decision is bound to the owner statement hash, full proof hash, permanent section tag, exact occurrence artifact, and pinned source revision, so unlisted uses of either section remain unresolved. The mapping does not repair omissions in the source proof: the functorial descent step for `0BB4`, finite-presentation detail for `07SK`, and bilinearity plus excluded-example dependency for `09CN` remain explicit debt; `06QZ` and `0DUJ` still have implicit prerequisites and independent review pending.

The previous source-audited local promotion turns the exact labeled Cauchy-Binet prose/list span into theorem-like claim Tag `0F0K` and resolves owner `07DQ`'s one explicit proof reference to it. The resulting edge is an ordinary explicit proof-xref edge; item labels are not promoted globally. Tag `0F0K` has no source proof and therefore no proof route, so it remains dependency-pending proof debt. Tag `07DQ` gains a captured route through the claim. Exact span, owner statement, full owner proof, source revision, and occurrence hashes guard the decision, and both records remain unreviewed.

The latest exact-label batch exercises three additional boundaries. First, source-unproved algebra items become route-less claims, while item `0GQT` and equation `06W6` become claims only because their exact spans contain derivations; `06W6` receives the audited 2-Yoneda dependency `004B`. Second, proof-used assumptions, definitions, constructions, and the single Chow remark `0F9Z` become support nodes with complete owner/count guards, without globally importing their environment classes. Support-level provenance that is not yet modeled as a direct edge is retained as learner-prerequisite debt, and owner omissions are copied into route evidence. Third, nine exact étale goal-item occurrences are suppressed as conclusions being proved instead of being converted into backwards edges. These policies are covered by exact-source regression fixtures and preserve the distinction among extraction, candidate dependency inference, proof debt, and independent review. The rejected example-heavy S0002 extraction remains absent. A root-manifest entry establishes ownership and queue identity without redistributing a physical graph, while a local candidate still cannot claim theorem coverage until its inventory and dependency decisions receive independent review.
