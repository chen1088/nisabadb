...
`data/knowledge/source-records.json` is only the fingerprint-locked index of 688 approved rows and their 717 required components. `data/books/manifest.json` is a generated, queue-only public aggregate whose ordered entries preserve every component identity and whose 717 `artifactPath` values remain null. Locally populated components use the canonical ignored path `data/books/S####/<component>.json`; their existence never changes the tracked manifest. Four local candidate graphs use the v1.1 sharded representation described below, while Git stores only their source-resolution metadata and hashes.
