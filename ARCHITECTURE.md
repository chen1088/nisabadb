...
`data/knowledge/source-records.json` is only the fingerprint-locked index of 688 approved rows and their 717 required components. `data/books/manifest.json` is a generated aggregate. The current v1 checkpoint isolates each component in `data/books/S####/<component>.json`; that boundary permits parallel extraction and review, but the 53 MB S0262 pilot proves that one physical JSON document per populated component is not a viable all-corpus format. The v1 files are migration inputs, not the permanent storage contract.
...
- **Knowledge / Coverage** is a temporary diagnostic for the two v1 extraction pilots, not a reader-facing product surface or a target for further raw-graph UI work. Phase-I progress will be reported from small manifests and command-line validation, while the raw graph shards remain data-only.
...
Before more dense components are imported, v1.1 will replace each populated monolith with a component directory. Its `component.json` will own identity, exact-edition metadata, workflow state, aggregate counts, distribution class, and an ordered shard index. Canonical LF-terminated JSONL shards will separately hold source units, unit inventories, nodes, external inputs, dependencies, proof routes, and unresolved source references. Dependencies, routes, and references are assigned to the source unit of their dependent or owner node; oversized unit partitions are split deterministically so no shard exceeds the configured size bound.

Every shard index entry records its relative path, schema version, record count, byte count, and SHA-256 digest. A component digest is computed over the ordered shard descriptors, and extraction and independent graph reviews bind to that digest. Records are sorted by stable IDs, and the first migration preserves existing per-entity evidence exactly rather than deduplicating it into shared mutable review objects. The root manifest continues to contain exactly 717 registry-derived component rows, but an awaiting-edition row may have a null component-manifest path instead of requiring an empty placeholder file.

Git stores the schemas, small manifests, reviewable JSONL graph data, and newly written normalized descriptions. It does not store source PDFs, EPUBs, or bulk verbatim text merely because an extractor can access them. Each component declares whether its derived records may be distributed; restricted source caches remain local or private while hashes, stable locators, structural edges, and permitted descriptions retain provenance. SQLite, DuckDB, search indexes, and compressed release/object-storage bundles may be generated for workers or analysis, but they are disposable derivatives and never the review authority.

Raw graph shards are not copied into the GitHub Pages artifact. If public graph dumps are later useful, a release or object store will publish versioned compressed snapshots with the same component and shard digests. The storage migration must prove node, edge, route, reference, identity, and aggregate-hash parity for S0262 first and S0060 second before the v1 monoliths are removed or extraction fans out across the remaining components.
