...
`data/knowledge/source-records.json` is only the fingerprint-locked index of 688 approved rows and their 717 required components. `data/books/manifest.json` is a generated aggregate. Each component keeps the stable index path `data/books/S####/<component>.json`. The two populated pilots now use the v1.1 sharded representation described below; the other 715 paths remain transitional v1 placeholders until the root index can represent an absent artifact directly.
