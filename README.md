...
The ignored local `S0262/complete-source.json` index and its component shards form the first dense-book checkpoint dataset. Under the descending-complexity work order, *The Stacks Project* is the first and highest-priority source book; it remains the active target, and no second-ranked successor has been selected or started. The candidate pins the official LaTeX source at tag-synchronized commit `ed88ff783bcb4dd9a28518a33b028841094009cf`, inventories all 116 chapters, and gives every included result its permanent Stacks tag. It contains 15,017 nodes: 13,147 theorem-like nodes (12,587 lemmas, 330 propositions, 214 theorems, and 16 exact-label source-audited claims) and 1,870 support nodes (1,730 definitions, 127 situations/assumptions, and 13 constructions). It also contains 11 typed external theorems, 36,350 proof-use edges, and 11,298 source or explicitly alternate proof routes. The edges comprise 36,091 explicit proof-xref edges and 259 owner-specific audited semantic edges: 71 named-result invocations, 5 curated-claim prerequisites, 12 primary-source bibliographic dependencies, 109 deictic-proof dependencies, 25 occurrence-level resolutions of a mixed recall remark, and 37 owner-specific section-delegation dependencies. The dataset contains zero example nodes: 449 example environments, 386 exercises, and 1,041 ordinary remark environments remain excluded; no item, equation, or remark category was bulk-imported. Exact audits suppress 12 notation, optional, or goal-statement proof xrefs and 11 attribution, corroboration, background, or example-based citation records instead of creating false dependencies. There are 2,530 remaining unresolved theorem-target records (1,271 unique permanent labels) and 6 unresolved bibliographic proof citations. The graph is reproducibly extracted but not independently reviewed or complete; 1,886 theorem-like nodes still have no route with a resolved candidate prerequisite.
...
The latest bounded audit adds six exact algebra basic-notion item claims (`00BV`, `0516`, `00C3`, `00C6`, `00C7`, and `00CD`), the derived internal-Hom claim `0GQT`, and pushforward formula `06W6`; only claims with an inline derivation receive a route, while the six source-unproved algebra claims remain explicit proof debt. It also promotes fifteen exact proof-used support spans: assumptions `0DC2`--`0DC4`; constructions `06GN`, `06SI`, `0DZJ`, `0F6I`, `0880`, `0B53`, `035M`, and `0F9Z`; and definitions `06CU`, `06D0`, `0EV6`, and `051U`. Tag `06W6` has an audited semantic prerequisite on the 2-Yoneda lemma `004B`; the promoted Chow remark `0F9Z` is a construction, not a theorem. Owner-specific omissions such as restriction-map compatibility, commutativity, the affine-cover argument, and the local identity used by `0GQT` remain written into route evidence. Nine occurrences of the étale-cohomology goal items under owners `0A5B`, `0A5D`, `0GJA`, and `03SC` are suppressed as conclusions being established rather than prerequisites. Every promotion or suppression is exact-label and owner-bounded; item, equation, and remark labels still receive no global alias, and the Stacks graph remains incomplete and unreviewed.
