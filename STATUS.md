...
| Candidate exact editions identified / administrator-verified | 5 / 0 |
| Components with complete theorem inventories | 0 / 717 |
| Components with independently reviewed dependency graphs | 0 / 717 |
| Candidate source units | 1,029 |
| Captured / reviewed source-unit inventories | 1,029 / 0 |
| Candidate theorem-like / support nodes | 14,323 / 2,597 |
| Candidate / reviewed source dependency edges | 39,677 / 0 |
| Unresolved source-reference records | 45 |

The provisional 126-chapter roadmap is not corpus coverage and is not a target to enlarge now. It is an early Phase-II hypothesis for a future compressed NisabaDB book. The 717 source components must first retain their own uncompressed theorem and support-node graphs under stable component identities. Five candidates now use ignored local sharded v1.1 storage; all 717 tracked manifest entries have absent artifacts. A generated exact-source queue covers all 717 identities, but its five sparse proposals are still candidates and zero editions or distributions are administrator-verified.
...
The second operational target is `S0321`, *The Open Logic Text*. Its complete-build candidate is pinned to the official LaTeX repository at commit `1e960beff9ed7835bf3e3f1335e21af3439cd107`. The source execution has 694 ordered import instances over 642 unique paths; the 52 repeated paths are intentional PL/FOL context instantiations and are not deduplicated. The graph contains 786 theorem-like nodes (182 theorems, 410 propositions, 121 lemmas, and 73 corollaries), 522 support nodes, and 155 exact proof-reference source artifacts. It records 700 candidate dependencies, 693 source-proof routes, and 892 references. Eighteen references remain unresolved, including eight proof xrefs, one proof-local citation, and one deliberately ambiguous duplicated label; 539 theorem-like nodes remain dependency-pending. An independent TeX-engine count matches the importer's formal-environment baseline, including 699 active proofs, and the source traversal has zero missing imports, import cycles, or unevaluated conditionals. The CC-BY-4.0 source proposal and all graph decisions remain unreviewed candidates.
...
- Current validation: lint passes; 24 test files / 232 tests pass; the combined 30-test graph/source-ledger/boundary check validates all 717 identities, five sparse source records, five ignored local graph candidates, 38 local shards, five candidate editions, zero verified editions or distributions, and both open license blockers. Git tracks only `data/books/manifest.json` under `data/books`; that manifest has zero payload paths, and the production build remains queue-only.
...
3. Source books are Phase-I graph units and non-omission evidence, not chapters of the eventual rewritten curriculum. Each required component owns a stable manifest identity; S0060, S0091, S0164, S0262, and S0321 now own ignored local sharded v1.1 data directories, while Git stores only metadata and hashes.
4. The 688 source rows expand to 717 required components. Five candidate editions have been identified, zero are administrator-verified, and none of the 717 components yet has a complete theorem inventory or independently reviewed source graph.
...
Default continuation order is the seven-step Phase-I sequence above. The ignored local v1.1 checkpoint has exact identity, record, evidence, reference, and logical-digest parity on S0060, S0091, S0164, S0262, and S0321. The tracked v1.2 graph manifest preserves all 717 identities with null artifact paths while separately reporting source-route and mathematical-dependency debt; the source-resolution manifest records five candidate editions and graph hashes without redistributing raw payloads. The first and second operational targets now have materialized raw passes; the next extraction may select the third source by the same reproducible-workload rule while existing mathematical-classification and independent-review debt stays explicit. Do not write `R003`, enlarge the 126-chapter roadmap, perform new compression work, or begin textbook-wide Lean 4 formulation during raw source-book graph construction.
...
- The exact approved registry contains 688 fingerprint-locked rows in 31 intake branches and 717 required components. Its v1.2 queue-only graph manifest has 0 payload artifacts and 717 null paths. Ignored local S0060, S0091, S0164, S0262, and S0321 use 38 content-addressed JSONL shards (largest 5,242,802 bytes) behind stable component indexes and contribute 1,029 source units, 14,323 theorem-like nodes, 2,597 mathematical support nodes, 1,392 raw source artifacts, and 39,677 candidate edges. All 717 graphs remain incomplete and unreviewed. The tracked source-resolution queue has five candidate exact-edition/importer-hash records, zero verified selections or distributions, and open license blockers on S0060 and S0164.
...
The current textbook records seven official registry-linked references spanning arithmetic, proof, discrete mathematics, sets, functions, abstract-algebra notation, and linear algebra. They are comparison evidence attached to rewritten nodes, not a public library, a list of assignments, or the architecture of Knowledge. The Phase-I source corpus now has 14,323 candidate theorem-like nodes across S0060, S0091, S0164, S0262, and S0321, but its complete and independently reviewed theorem-inventory count remains 0 of 717 components; the seven lesson references do not add to that count.
