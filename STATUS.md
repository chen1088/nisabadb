...
The dense-book checkpoint is `S0262`, *The Stacks Project*. It is the first and highest-priority book under the descending-complexity work order; it remains the active target, and no second-ranked successor has been selected or started. The official LaTeX source is pinned at tag-synchronized commit `ed88ff783bcb4dd9a28518a33b028841094009cf`, because the later upstream head contains six new lemmas not yet assigned permanent Stacks tags. The importer inventories all 116 chapters and captures 15,017 nodes: 13,147 theorem-like nodes, 1,870 support nodes, and 11 typed external theorems. The theorem-like nodes comprise 12,587 lemmas, 330 propositions, 214 theorems, and 16 exact source-audited claims: six exact-label remarks, eight labeled list items, and two labeled prose/display spans. Twenty-five exact proof-used assumption/definition/construction spans are promoted as support nodes; all 449 example environments, 386 exercises, and the other 1,041 remark environments remain excluded. The graph has 36,350 proof-use edges and 11,298 candidate source or alternate proof routes. Its edges comprise 36,091 explicit proof-xref edges plus 259 exact owner-specific audited semantic edges (71 named-result, 5 curated-claim, 12 external-citation, 109 deictic, 25 occurrence-level recall-bundle, and 37 section-delegation dependencies). Exact audits suppress 12 notation, optional, or goal-statement proof xrefs and 11 nondependency citations. It retains 2,530 unresolved theorem-target records across 1,271 unique permanent labels plus 6 bibliographic proof-citation records; 1,886 theorem-like nodes remain without a route containing a resolved candidate prerequisite. This is a reproducible extracted graph, not an independently reviewed or complete one.
...
The latest bounded S0262 audit promotes six exact source-unproved algebra basic-notion items (`00BV`, `0516`, `00C3`, `00C6`, `00C7`, and `00CD`) as route-less claims, plus derived claim `0GQT` and pushforward formula `06W6` with their inline derivations. Tag `06W6` receives one exact semantic dependency on the 2-Yoneda lemma `004B`. Fifteen further proof-used spans become support nodes: three assumptions, four definitions, and eight constructions, including the single promoted Chow remark `0F9Z`. The importer records support provenance and owner-specific omissions as explicit learner or route debt instead of declaring those paths closed. Nine exact references from owners `0A5B`, `0A5D`, `0GJA`, and `03SC` to étale-cohomology goal items are suppressed because they name conclusions being established, not prerequisites. Exact source-span and complete owner/count guards prevent these decisions from becoming global item, equation, or remark aliases. This batch removes sixty unresolved tagged records, but 2,530 tagged and 6 bibliographic records still remain; it is progress within the first book, not completion and not permission to start a second.
...
- Validated correction: lint passes; 21 test files / 207 tests pass; the combined 29-test graph/source-ledger/boundary check validates all 717 identities, four sparse source records, four ignored local graph candidates, 30 local shards, four candidate editions, zero verified editions or distributions, and both open license blockers. Git tracks only `data/books/manifest.json` under `data/books`; that manifest has zero payload paths, and the production build remains queue-only.
...
Default continuation order is the seven-step Phase-I sequence above. The ignored local v1.1 checkpoint has exact identity, record, evidence, reference, and logical-digest parity on S0060, S0091, S0164, and S0262. The tracked graph manifest preserves all 717 identities with null artifact paths; the separate source-resolution manifest records four candidate editions and graph hashes without redistributing raw payloads. Do not write `R003`, enlarge the 126-chapter roadmap, perform new compression work, or begin textbook-wide Lean 4 formulation while source-book graph construction is the active phase.
...
- The exact approved registry contains 688 fingerprint-locked rows in 31 intake branches and 717 required components. Its queue-only graph manifest has 0 payload artifacts and 717 null paths. Ignored local S0060, S0091, S0164, and S0262 use 30 content-addressed JSONL shards (largest 5,242,723 bytes) behind stable component indexes and contribute 335 source units, 13,527 theorem-like nodes, 2,051 support nodes, and 36,423 candidate edges. All 717 graphs remain incomplete and unreviewed. The tracked source-resolution queue has four candidate exact-edition/importer-hash records, zero verified selections or distributions, and open license blockers on S0060 and S0164.
