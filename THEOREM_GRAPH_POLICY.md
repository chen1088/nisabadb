...
Deterministic extraction creates candidates, not reviewed mathematics. A component becomes `reviewed-complete` only when an independent reviewer confirms every source-unit inventory, every included node, every dependency/root decision, and every unresolved external or excluded target. Compression and simplification begin only after the source-local graph gate passes.
