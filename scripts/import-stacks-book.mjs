...
function usage() {
  return [
    ...
    "The importer inventories formal mathematical nodes and exact proof-referenced raw source artifacts separately.",
    "Examples, exercises, remarks, equations, items, and sections remain outside the mathematical graph unless an exact source-audited claim/support span promotes them.",
  ].join("\n");
}
...
function report({ recordId, componentId, commit, capturedAt, sourceRepository, write, destination, stats }) {
  process.stdout.write([
    ...
    `Graph node kinds: ${JSON.stringify(stats.kindCounts)}`,
    `Raw source-artifact kinds: ${JSON.stringify(stats.sourceArtifactKindCounts)}`,
    ...
    `  Raw source-reference edges: ${stats.sourceArtifactDependencyCount}`,
    ...
    `  Routes containing raw source artifacts: ${stats.sourceArtifactRouteCount}`,
    `  Captured empty source routes (not roots): ${stats.emptySourceRouteCount}`,
    ...
    `Unique unresolved tagged proof targets: ${stats.uniqueUnresolvedTaggedProofTargetCount}`,
    ...
    `Theorem-like nodes with no captured source route: ${stats.unroutedTheoremCount}`,
